  ...
  //      if (profileDTO.getNominee() != null && profileDTO.getNominee().size() > 1) {
    //        throw new Only1NomineeAllowedException("Only 1 nominee is allowed per user.");
      //  }
        ...
        if (!"Indian".equalsIgnoreCase(profileDTO.getNationality()) && !"Passport".equalsIgnoreCase(profileDTO.getIdProof())) {
            throw new IllegalArgumentException("If nationality is not Indian, the only acceptable ID proof type is Passport.");
        }
    }

    private void generateUsername(UserprofileDTO profileDTO) {
